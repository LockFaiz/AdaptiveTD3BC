...
if __name__ == "__main__":
    ...
    if args.load_policy_id != "":
        policy_file = f'{args.policy}_{args.env}' if args.load_policy_id == "default" else f'{args.policy}_{args.env}_{args.seed}_{args.load_policy_id}'
        ...
        with wandb.init(project='adaptive_bc', group=args.env, job_type="pretrain", name=run_name):
            ...
            pretrain_evaluations = {'d4rl': [], 'evaluation': [], 'length': []}    # 定义dict
            for i in tqdm.tqdm(range(args.pretrain_timesteps)):
                ...
                if i % args.eval_freq == 0:
                    ...
                    np.save(f"./results/{args.env}_pretrain_{args.seed}", pretrain_evaluations)
            # save policy
    ...
    with wandb.init(project='adaptive_bc', group=args.env, job_type="finetune", name=run_name):
        ...
        finetune_evaluations = {'d4rl': [], 'evaluation': [], 'length': []}  # 定义dict
        episode_return = 0.
        for t in tqdm.tqdm(range(args.max_timesteps)):
            ...
            if t % args.eval_freq == 0:
                ...
                # save pretrain_evaluations to 'results'
                np.save(f"./results/{args.env}_finetune_{args.seed}", finetune_evaluations)
